...
This script downloads ERA5 data from the Analysis-Ready, Cloud-Optimized (ARCO) ERA5 dataset.
It provides access to both pressure level and single-level variables with flexible options.
"""
        ...
        try:
            ...
            if is_multilevel:
                for level in var_data.level.values:
                    ...
                    filename = f"{var_name}_{int(level)}_{year}{month:02d}{day:02d}.nc"
                ...
                filename = f"{var_name}_sfc_{year}{month:02d}{day:02d}.nc"
                output_file = os.path.join(output_dir, filename)
